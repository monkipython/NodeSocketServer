    ...
    config: {// "tags" enable swagger to document API 
        ...
        validate: {
            payload:
            {
                ...
                ent_last_name: Joi.any().description('Last Name'),
    ...
    config: {// "tags" enable swagger to document API 
        ...
        validate: {
            payload:
            {
                ...
                ent_device_type: Joi.string().required().description('1-ANDORID, 2-IOS, 3-WEBSITE'),
    ...
    config: {// "tags" enable swagger to document API 
        ...
        validate: {
            payload:
            {
                ...
                ent_reason: Joi.string().required().description('Cancellation reason')
            }
        }
    },
...
//get all support text
server.route({
    method: 'get', // Methods Type
    path: '/slave/support_text/{ent_sess_token}', // Url
    config: {// "tags" enable swagger to document API 
        tags: ['api', 'slave'],
        description: 'get all Support text',
        notes: "get all Support text", // We use Joi plugin to validate request 
        plugins: {
            'hapi-swagger': {
                payloadType: 'form'
            },
    ...
    handler: slaveModule.supportText
});

//get all Cancel reasons
server.route({
    method: 'get', // Methods Type
    path: '/slave/cancel_reason/{ent_sess_token}', // Url
    config: {// "tags" enable swagger to document API 
        tags: ['api', 'slave'],
        description: 'get all Cancel Reasons',
        notes: "get all Cancel Reasons", // We use Joi plugin to validate request 
        plugins: {
            'hapi-swagger': {
                payloadType: 'form'
            },
    ...
    handler: slaveModule.cancelReason
});
...
server.route({
    method: 'get', // Methods Type
    path: '/slave/resetWithPhone/{ent_country_code}/{ent_mobile}', // Url
    config: {// "tags" enable swagger to document API 
        tags: ['api', 'slave'],
        description: 'reset password with Phone Number',
        notes: "reset password with Phone Number", // We use Joi plugin to validate request 
        plugins: {
            'hapi-swagger': {
                payloadType: 'form'
            },
        ...
        validate: {
            params:
            {
                ent_mobile: Joi.any().required().description('Mobile'),
                ent_country_code: Joi.any().required().description('country code')
            }
        }
    },
    handler: slaveModule.resetWithPhone
});

server.route({
    method: 'get', // Methods Type
    path: '/slave/forgetPassword/{ent_password}/{ent_token}', // Url
    config: {// "tags" enable swagger to document API 
        tags: ['api', 'slave'],
    ...
    handler: slaveModule.forgetPassword
});

server.route({
    method: 'put', // Methods Type
    path: '/slave/changePassword/{ent_token}', // Url
    config: {// "tags" enable swagger to document API 
        tags: ['api', 'slave'],
        description: 'reset new password with Email',
        notes: "reset new password with Email", // We use Joi plugin to validate request 
        ...
        validate: {
            ...
            payload: {
                ent_old_password: Joi.string().required().description('enter old password'),
                ent_new_password: Joi.string().required().description('enter new password')
            }
        }
    },
...
server.route({
    method: 'get', // Methods Type
    path: '/slave/verifyCode/{ent_mobile}/{ent_country_code}/{ent_code}/{ent_service}', // Url
    config: {// "tags" enable swagger to document API 
        tags: ['api', 'slave'],
        ...
        validate: {
            params:
            {
                ent_mobile: Joi.string().required().description('mobileNo without country code'),
                ent_country_code: Joi.string().required().description('country code'),
    ...
    config: {// "tags" enable swagger to document API 
        ...
        validate: {
            payload:
            {
                ...
                ent_status: Joi.any().required().description('1-BOOK , 2- BOOK & CONFIRM'),
                ent_slot_id: Joi.any().description('Slot id')
            }
        }
    },
...
// =============== CUSTOMER MODULE ========================================================================== //
var customerModule = require('./models/customer');

// =============== CUSTOMER SIGNUP API =================
server.route({

    method: 'POST', // METHOD TYPE
    path: '/customer/signup', // URL
    config: {

        // FOR VALIDATION AND DOCUMENTATION
        tags: ['api', 'customer'],
        description: 'Customer signup',
        notes: "Customer signup for regestering through app.", // We use Joi plugin to validate request 
        plugins: {
            'hapi-swagger': {
                payloadType: 'form'
            },
        ...
        // REQUEST BODY, HEADER, QUERY, PARAMETERS VALIDATIONS
        validate: {

            //REQUEST BODY VALIDATION
            payload:
            {
                first_name: Joi.string().required().description("Customer's first name").default("Pallavi"),
                last_name: Joi.any().description("Customer's last name").default("Verma"),
                email: Joi.string().required().description("Customer's email").default("vermapallavi93@gmail.com"),
                country_code: Joi.string().required().description("Customer's country's phonecode").default("+91"),
                mobile: Joi.string().required().description("Customer's phone number").default("9934237921"),
                latitude: Joi.string().required().description("Customer's location's latitude").default("13.3423423"),
                longitude: Joi.string().required().description("Customer's location's longitude").default("77.3432423"),
                device_id: Joi.string().required().description("ID of Customer's device").default("12121212"),
                device_type: Joi.number().required().description('1-iOS, 2-Android').default('1'),
                profile_pic: Joi.any().description("Customer's profile picture url").default("http://iserve.ind.in/superadmin/theme/icon/green.png"),
                password: Joi.any().required().description("Customer's profile password").default('123456Aa'),
                zipcode: Joi.any().required().description("Costomer's address zipcode").default('812005'),
                signup_type: Joi.number().required().description("1-> Normal signup, 2-> Facebook Signup").default("1"),
                fb_id: Joi.any().description("If signup type is 2, then pass fbid").default("1234dfwerws4223"),

            },

            //REQUEST HEADER VALIDATION FOR BASIC AUTH 
            headers: Joi.object({
                'authorization': Joi.string().required().description('Basic Authentication')
            }).unknown().default({
                "authorization": "aXNlcnZlVXNlcjojaXNlcnZlVXNlcmpsa3NkaGtqZA=="
            })

        }
    },
    handler: customerModule.signup
});
...
server.route({
    ...
    method: 'POST', // METHOD TYPE
    path: '/customer/login', // URL
    config: {

        // FOR VALIDATION AND DOCUMENTATION
        tags: ['api', 'customer'],
        description: 'Customer Login',
        notes: "Customer Login for regestering through app.", // We use Joi plugin to validate request 
        plugins: {
            'hapi-swagger': {
                payloadType: 'form'
            },
        ...
        validate: {
            ...
            payload:
            {
                email: Joi.any().description("Customer's email"),

                mobile: Joi.any().description("Customer's phone number"),
                password: Joi.any().required().description("Customer's profile password").default('123456Aa')
            },
...
server.route({
    ...
    method: 'POST', // METHOD TYPE
    path: '/customer/emailValidation', // URL
    config: {

        // FOR VALIDATION AND DOCUMENTATION
        tags: ['api', 'customer'],
        description: "Customer's Email Validation",
        notes: "Customer's email validation check, checking if the email is registred or not.", // We use Joi plugin to validate request 
        plugins: {
            'hapi-swagger': {
                payloadType: 'form'
            },
        ...
        validate: {
            ...
            payload:
            {
                email: Joi.string().required().description("Customer's email")
            },
    ...
    handler: customerModule.emailValidation
});

// =============== CUSTOMER PHONE NUMBER VALIDATION API ================
server.route({

    method: 'POST', // METHOD TYPE
    path: '/customer/phoneValidation', // URL
    config: {

        // FOR VALIDATION AND DOCUMENTATION
        tags: ['api', 'customer'],
        description: "Customer's Phone Validation",
        notes: "Customer's phone number validation check, checking if the phone number is valid or not is registred or not.", // We use Joi plugin to validate request 
        plugins: {
            'hapi-swagger': {
                payloadType: 'form'
            },
        ...
        validate: {
            ...
            payload:
            {
                mobile: Joi.string().required().description("Customer's phone")
            },
    ...
    handler: customerModule.phoneNumberValidation
});

// =============== CUSTOMER --> SEND OTP ON REGISTERED MOBILE NUMBER API ================
server.route({

    method: 'POST', // METHOD TYPE
    path: '/customer/sendOtp', // URL
    config: {

        // FOR VALIDATION AND DOCUMENTATION
        tags: ['api', 'customer'],
        description: "Send OTP on Customer's mobile number",
        notes: "Send OTP on Customer's mobile number.", // We use Joi plugin to validate request 
        plugins: {
            'hapi-swagger': {
                payloadType: 'form'
            },
        ...
        validate: {
            ...
            {
                mobile: Joi.string().required().description("Customer's phone"),
                country_code: Joi.string().required().description("Customer's country code")
            },
    ...
    handler: customerModule.sendOtp
});

// ============== VERIFY OTP FOR SIGNUP AND FORGET PASSWORD API =================
server.route({

    method: 'POST', // METHOD TYPE
    path: '/customer/verifyOtp', // URL
    config: {
        ...
        validate: {
            ...
            payload:
            {
                mobile: Joi.string().required().description("Customer's phone number").default('9934254678'),
                country_code: Joi.string().required().description("Country code").default('+91'),
                code: Joi.number().required().description("OTP Code").default('1234'),
                verificationType: Joi.number().required().description('1-> Signup, 2-> ForgetPassword').default('1')
            },
    ...
    handler: customerModule.verifyOtp
...
server.route({
    ...
    method: 'POST', // METHOD TYPE
    path: '/customer/forgetPassword', // URL
    config: {
    ...
    handler: customerModule.forgetPassword
